...
const values = [
  ...
  { number: null, plurals: ['строка', 'строки', 'строк'], result: 'строк' },
...
test('Test filter plural in cycle', (t) => {
  values.forEach((record) =>
    test.it(`plural ${record.number} ${record.result}`, () => {
      assert.equal(plural(record.number, record.plurals), record.result);
    }))
});
